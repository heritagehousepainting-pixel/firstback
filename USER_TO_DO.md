...
the last only when you're ready to put RingBack online for real.
...
- **Password:** `ringback123`  ← change this
...
5. Enter the current password (`ringback123`) and a new one (8+ characters), then **Update password**.
...
2. Top bar → **project dropdown → New Project** (name it e.g. "RingBack"), then
   select it.
3. Left menu → **APIs & Services → Library** → search **"Google Calendar API"** →
   open it → **Enable**.
4. Left menu → **APIs & Services → OAuth consent screen**:
   - Choose **External**, click Create.
   - Fill in app name ("RingBack"), your email where required, **Save and continue**.
...
5. Left menu → **APIs & Services → Credentials → Create credentials → OAuth client ID**:
   ...
   - Click **Create**. Google shows you a **Client ID** and a **Client secret** —
     keep this box open.
6. Open the file `ringback/.env` in a text editor and add these two lines
...
> Note: when you later host RingBack on a real web address (not `127.0.0.1`),
...
3. Open `ringback/.env` and add these three lines (your real values; the number
   in `+1...` E.164 form):
   ```
...
5. RingBack has to be reachable on the internet for Twilio to call it. For testing,
   ...
   ```
   RINGBACK_PUBLIC_URL=https://your-public-address
   ```
6. In the Twilio Console, open your number and set its webhooks (both **POST**):
   ...
   answered by the AI. (RingBack only accepts requests Twilio actually signed.)
...
```
RINGBACK_VOICE_URL=https://your-voice-service-address
```
...
> RingBack is built to stay on the safe side (the text-back is informational, the AI
...
RingBack can alert you the moment a lead comes in or an estimate books. By
...
RingBack now texts a reminder before each booked estimate and sends **one** gentle
...
> `RINGBACK_TASKS_SECRET` in `.env` and have cron `POST /tasks/run-due` every
> minute with that secret in an `X-Tasks-Secret` header (belt and suspenders).

---

## ☁️ Put RingBack online (Render)

This puts RingBack on the internet so you (and later real customers) can reach it.
...
**1. Put the code on GitHub** (Render deploys from a Git repo).
   - Make a free account at **https://github.com** and create a **new private repo**
     named e.g. `ringback` (don't add any files to it).
   - In a terminal in the `ringback` folder, push the code (ask me and I'll do the
...
**2. Create the Render service.**
   - Sign up at **https://render.com** → **New + → Blueprint** → connect GitHub and
     pick the `ringback` repo. Render reads `render.yaml` and sets up the web
...
**3. Add your AI brain key** (in the service → **Environment**):
   - Claude: `RINGBACK_PROVIDER=claude` and `ANTHROPIC_API_KEY=your-key`, or
   - MiniMax: `RINGBACK_PROVIDER=minimax` and `MINIMAX_API_KEY=your-key`.
   - Also set `RINGBACK_TZ` to your timezone (e.g. `America/New_York`).
...
**4. Open your URL** (Render gives you something like
   `https://ringback.onrender.com`).

**5. Log in and lock it down.** It's a fresh database (no demo leads). Log in with
   the starter account (`heritagehousepainting@gmail.com` / `ringback123`) and
   **change the password** in Settings right away. (Or set `RINGBACK_OWNER_PASSWORD`
...
## 🚀 Before you put RingBack online (production checklist)
...
2. Add it to `ringback/.env`:
   ```
   RINGBACK_SECRET=the-long-random-value-you-just-generated
   ```
3. Once you're serving over **HTTPS**, also add `RINGBACK_HTTPS=1` so the login
...
stay correct:
1. Add to `ringback/.env` (use your IANA zone name):
   ```
   RINGBACK_TZ=America/New_York
   ```
...
## Notes / housekeeping
- Backup files named `ringback.db.bak-*` were created before risky changes. Once
  you're happy everything works, they're safe to delete.
- Deleting `ringback.db` resets everything (re-seeds Heritage + the starter login).
